...
def load_name_data(path):
    ...
    with open(path) as data_file:
        lines = [line for line in data_file.read().encode("utf-8").decode("utf-8").split("\n") if line.strip() != ""]
        for line in lines:
            fields = line.split("\t")
...
def load_name_stroke_data():
    stroke_data = get_items_in_file(get_relative_path(__file__, "data/stroke_data"))
    stroke_counts = list(map(lambda count: int(count), stroke_data))
    sorted(stroke_counts)
    # print stroke_counts
    return stroke_counts


def get_stroke_score(stroke):
    normalized = 1 - ((list(all_names_stroke_count).index(stroke) + 1) * 1.0 / len(list(all_names_stroke_count)))
